...
export default function AddComplaintPage() {
  ...
  const [errors, setErrors] = useState([]); // unified errors array for validation
  ...
  const validateComplaintText = (title, desc) => {
    ...
    // Character-repetition spam check (kept)
    if (/(.)\1{29,}/.test(cleanDesc))
      errors.push(
        "Description has excessive repetition of a single character — please revise."
      );

    // All word-based (token) checks have been removed.

    // Ensure title has at least one visible alphanumeric character
    if (!/[A-Za-z0-9]/.test(cleanTitle)) {
      errors.push("Title must contain at least one letter or number.");
    }

    // Ensure description has at least one visible alphanumeric character
