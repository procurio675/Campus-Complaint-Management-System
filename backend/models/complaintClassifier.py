...
Also saves the same JSON to ./out/complaint_<timestamp>.json

Usage (PowerShell):
  python .\AI-LLM\complaintClassifier.py ^
...
if USE_LLM:
    ...
    SYSTEM_PROMPT = (
        ...
        "Never include explanations, rationale, or extra fields. Only JSON."
    )
...
def analyze_complaint(title: str, body: str) -> dict:
    """
    Primary callable for Node/Express or other Python code.
    Returns: {"committee": "...", "priority": "..."}
    Uses LLM when available; otherwise falls back to rules.
    """
    # Fallback (guardrail) results
    rule_comm = _rule_committee(title, body) or "Academic"
    rule_prio = _rule_priority(title, body) or "Medium"

    if not USE_LLM:
        return {"committee": rule_comm, "priority": rule_prio}

    try:
        res = chain.invoke({"title": title, "body": body})
        committee = res.get("committee") or rule_comm
        priority  = res.get("priority")  or rule_prio
        return {"committee": committee, "priority": priority}
    except Exception as e:
        # Safe fallback
        sys.stderr.write(f"[LLM fallback] {e}\n")
        return {"committee": rule_comm, "priority": rule_prio}
...
def main():
    ...
    out = analyze_complaint(args.title, args.body)

    # Print clean JSON to stdout (so Node can capture it)
    print(json.dumps(out, ensure_ascii=False))

    # Save JSON file automatically (use absolute path based on script location)
    try:
        script_dir = Path(__file__).resolve().parent
        out_dir = script_dir.parent / "out"  # backend/out
        out_dir.mkdir(exist_ok=True)
        file_path = out_dir / f"complaint_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(out, f, indent=4, ensure_ascii=False)
    except Exception as e:
        # Don't fail if file saving fails - stdout output is what matters
        sys.stderr.write(f"[Warning] Could not save output file: {e}\n")
